...
SELECT first_name FROM sakila.actor   -- Agrupa todos os que possuem first_name igual na tabela sakila.actor
GROUP BY first_name;
...
-- HAVING --
-- Como um select dentro de um group by agrupado.
...
-- para que somente os nomes que estão cadastrados mais de duas vezes sejam exibidos.
...
-- É importante entender que quando usamos o HAVING
--  estamos filtrando somente os resultados gerados após o GROUP BY ter sido executado.